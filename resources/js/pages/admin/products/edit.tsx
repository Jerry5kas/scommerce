...
export default function AdminProductsEdit({ product, verticalOptions, categories, collections }: AdminProductsEditProps) {
    ...
    const handleRemoveExistingImage = (index: number) => {
        const current = Array.isArray(form.data.images) ? form.data.images : [];
                ...
                <form onSubmit={submit} className="max-w-2xl space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <select required className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" value={form.data.category_id} onChange={(e) => form.setData('category_id', Number(e.target.value))}>
                                {categories.map((c) => (
                    ...
                    <div>
                        ...
                        {(imagePreview || imageFile) && (
                            <div className="mb-3 relative inline-block">
                                ...
                                <button type="button" onClick={() => { setImageFile(null); setImagePreview(product.image ?? null); form.setData('image', product.image ?? ''); }} className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600" aria-label="Remove"><X className="h-4 w-4" strokeWidth={2} /></button>
                            </div>
                        )}
                        <div className="mb-3">
                    ...
                    <div>
                        ...
                        {Array.isArray(form.data.images) && form.data.images.length > 0 && (
                            <div className="mb-3 flex flex-wrap gap-3">
                                {form.data.images.map((src, index) => (
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm" value={form.data.unit} onChange={(e) => form.setData('unit', e.target.value)} />
                        </div>
                        <div>
