...
interface CollectionData {
    ...
    category_id: number | null; banner_image: string; banner_mobile_image: string | null;
    display_order: number; is_active: boolean; vertical: string;
    starts_at: string | null; ends_at: string | null; link_url: string | null;
...
export default function AdminCollectionsEdit({ collection, verticalOptions, categories }: AdminCollectionsEditProps) {
    ...
    const form = useForm({
        ...
        category_id: collection.category_id, banner_image: collection.banner_image,
        banner_mobile_image: collection.banner_mobile_image ?? '',
        display_order: collection.display_order, is_active: collection.is_active,
        vertical: collection.vertical ?? 'both',
        starts_at: toDatetimeLocal(collection.starts_at), ends_at: toDatetimeLocal(collection.ends_at),
        link_url: collection.link_url ?? '', meta_title: collection.meta_title ?? '',
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (bannerImageFile) {
                ...
                form.setData('banner_image', url); setBannerImageFile(null); setBannerImagePreview(url);
            }
            if (bannerMobileImageFile) {
                const url = await uploadImageToAdmin(bannerMobileImageFile, 'collections', csrfToken);
                form.setData('banner_mobile_image', url); setBannerMobileImageFile(null); setBannerMobileImagePreview(url);
            }
        ...
        <AdminLayout title={'Edit: ' + collection.name}>
            ...
            <form onSubmit={submit} className="space-y-6">
                    ...
                    <div className="grid gap-5 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            {(bannerImagePreview || bannerImageFile) && (
                                <div className="mb-3 relative inline-block">
                                    <img src={bannerImageFile ? (bannerImagePreview ?? '') : (bannerImagePreview || form.data.banner_image)} alt="" className="h-28 w-auto max-w-full rounded-lg border border-gray-200 object-cover" />
                                    <button type="button" onClick={() => { setBannerImageFile(null); setBannerImagePreview(collection.banner_image ?? null); form.setData('banner_image', collection.banner_image ?? ''); }} className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600"><X className="h-4 w-4" /></button>
                                </div>
                            )}
                            ...
                            <input type="text" placeholder="https://..." className={inputCls} value={form.data.banner_image} onChange={(e) => { form.setData('banner_image', e.target.value); setBannerImagePreview(e.target.value || (collection.banner_image ?? null)); if (e.target.value) setBannerImageFile(null); }} disabled={!!bannerImageFile} />
                        ...
                        <div>
                            ...
                            {(bannerMobileImagePreview || bannerMobileImageFile) && (
                                <div className="mb-3 relative inline-block">
                                    <img src={bannerMobileImageFile ? (bannerMobileImagePreview ?? '') : (bannerMobileImagePreview || form.data.banner_mobile_image)} alt="" className="h-28 w-auto max-w-full rounded-lg border border-gray-200 object-cover" />
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Link href={'/admin/collections/' + collection.id} className="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</Link>
                </div>
            </form>
        </AdminLayout>
    );
